...
export default function EventCommunitySection({ event, isOwner = false }) {
  ...
  const remove = async (postId) => {
    if (!(await showConfirmation({ title: "Excluir publicação?", text: "Essa ação não pode ser desfeita. Se for uma publicação principal, as respostas também serão excluídas.", confirmButtonText: "Excluir" }))) return;
    setBusy(true);
