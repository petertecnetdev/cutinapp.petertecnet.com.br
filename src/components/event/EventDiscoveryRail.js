    ...
    <section className={["cut-event-discovery", className].filter(Boolean).join(" ")}>
      <div className="cut-event-discovery__head">
        ...
        <div className="cut-event-discovery__actions">
          {visibleEvents.length > 1 && <div className="cut-event-discovery__controls" aria-label={`Navegar em ${title}`}><button type="button" onClick={() => scroll(-1)} aria-label="Ver anteriores"><i className="fa-solid fa-chevron-left" /></button><button type="button" onClick={() => scroll(1)} aria-label="Ver próximos"><i className="fa-solid fa-chevron-right" /></button></div>}
          {allTo && <Link to={allTo}>{allLabel} <i className="fa-solid fa-arrow-right" /></Link>}
        </div>
      </div>
            ...
            return (
              <article className="cut-event-discovery__card" key={eventStableKey(event)}>
                <Link to={eventHref} className="cut-event-discovery__mainLink" aria-label={`Ver evento ${event?.title || "Cutinapp"}`}>
                  <div className="cut-event-discovery__media"><EventArtwork image={event?.image} title={event?.title} alt={event?.title || "Evento Cutinapp"} loading={index === 0 ? "eager" : "lazy"} fetchPriority={index === 0 ? "high" : "auto"} decoding="async" />{dateMeta.dateTime ? <time dateTime={dateMeta.dateTime}>{dateMeta.label}</time> : <span>{dateMeta.label}</span>}</div>
