...
export default function ProductionCommunitySection({ production, isOwner = false }) {
  ...
  return <section className="cut-community cut-production-community" id="comunidade">
    ...
    {loading ? <ProcessingIndicatorComponent fullscreen={false} label="Carregando publicações" /> : posts.length === 0 ? <div className="cut-community-empty"><i className="fa-regular fa-comments" /><strong>A conversa ainda não começou</strong><span>Seja a primeira pessoa a publicar algo sobre esta produção.</span></div> : <div className="cut-community-list">{posts.map((post) => <article className="cut-community-post" key={post.id}>
