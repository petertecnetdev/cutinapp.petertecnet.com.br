...
export default function LocationFields({ value, onChange, required = false, showPublicToggle = false }) {
  ...
  const searchCity=(text)=>{
    onChange({ ...value, city:text, city_id:"", uf:"" }); setCities([]); clearTimeout(timer.current); setNotice("");
    if(text.trim().length<2)return;
    timer.current=setTimeout(async()=>{setLoadingCities(true);try{setCities(await cutinappService.locationCities("",text.trim()));}catch{setNotice("Não foi possível consultar as cidades brasileiras agora.");}finally{setLoadingCities(false);}},300);
  };
  const chooseCity=(city)=>{onChange({ ...value, city:city.name, city_id:String(city.ibge_code), uf:city.uf });setCities([]);setNotice("");};
  const lookupCep=async()=>{
    ...
    catch{setNotice("Não conseguimos consultar o CEP. Confira o número ou preencha o endereço e selecione a cidade pela lista oficial.");}
  ...
  return <div className="cut-location-fields" data-pt-location-native="true">
    ...
    <Row className="g-3">
      ...
      <Col md={3}><Form.Group><Form.Label>UF {required&&"*"}</Form.Label><Form.Control value={value.uf||""} readOnly placeholder="UF" aria-label="UF definida pela cidade" required={required}/><Form.Text>Preenchida pela cidade ou pelo CEP.</Form.Text></Form.Group></Col>
