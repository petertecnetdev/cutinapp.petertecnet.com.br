...
const withOrganizationAliases = (data = {}) => ({
  ...data,
...
const cutinappService = {
  ...
  publicProductions: async (params = {}) => withOrganizationAliases((await apiClient.get("/organizations/public", { params })).data),
  getProduction: async (id) => (await apiClient.get(`/organizations/${id}`)).data.organization,
  publicProduction: async (slug) => withOrganizationAliases((await apiClient.get(`/organizations/public/${slug}`)).data),
  createProduction: async (formData) => withOrganizationAliases((await apiClient.post("/organizations", formData)).data),
  updateProduction: async (id, formData) => withOrganizationAliases((await apiClient.patch(`/organizations/${id}`, formData)).data),
  ...
  eventCourtesies: async (eventId) => {
    const response = (await apiClient.get(`/events/${eventId}/tickets`)).data;
    return {
      ...response,
      tickets: (response.tickets || []).filter((ticket) => ticket.type === "courtesy" || Number(ticket.price || 0) === 0),
    };
  },
