...
const commerceService = {
  ...
  syncPayment: async (publicId) => (
    await apiClient.post(`/commerce/orders/${publicId}/sync-payment`)
  ).data.order,

  purchases: async (params = {}) => (
    await apiClient.get("/commerce/purchases", { params })
  ).data,
  purchase: async (publicId) => (
    await apiClient.get(`/commerce/purchases/${publicId}`)
  ).data.order,
  receipt: async (publicId) => (
    await apiClient.get(`/commerce/purchases/${publicId}/receipt`)
  ).data.receipt,
  ...
  saveEventItem: async (eventId, payload, itemId = null) => (
    itemId
      ? await apiClient.put(`/events/${eventId}/items/${itemId}`, payload)
  ...
  paymentAccount: async (organizationId) => (
    await apiClient.get(`/organizations/${organizationId}/payment-account`)
  ).data.account,
  connectMercadoPago: async (organizationId) => (
    await apiClient.get(`/organizations/${organizationId}/payment-provider/connect`)
  ).data,
  financialSummary: async (organizationId) => (
    await apiClient.get(`/organizations/${organizationId}/financial-summary`)
  ).data,
};
