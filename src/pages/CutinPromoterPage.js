...
export default function CutinPromoterPage(){
 ...
 const copy=async campaign=>{const url=`${window.location.origin}/evento/${campaign.event_id}?ref=${campaign.code}`;try{await navigator.clipboard.writeText(url)}catch(copyError){setError("Não foi possível copiar o link automaticamente.")}};
