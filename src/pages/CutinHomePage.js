...
const date=v=>v?new Intl.DateTimeFormat("pt-BR",{dateStyle:"medium",timeStyle:"short"}).format(new Date(v)):"Data a confirmar";

export default function CutinHomePage(){
  const [events,setEvents]=useState([]); const [loading,setLoading]=useState(true);
  useEffect(()=>{CutinService.events({per_page:6}).then(v=>setEvents(Array.isArray(v)?v.slice(0,6):[])).catch(()=>setEvents([])).finally(()=>setLoading(false));},[]);
  return <CutinLayout>
