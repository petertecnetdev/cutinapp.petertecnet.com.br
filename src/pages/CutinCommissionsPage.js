...
export default function CutinCommissionsPage(){
 ...
 const load=async()=>{try{const list=await CutinService.promoters(eventId);const rows=Array.isArray(list)?list:[];setPromoters(rows);const pairs=await Promise.all(rows.map(async p=>{try{return[p.id,await CutinService.promoterCommissions(eventId,p.id)]}catch{return[p.id,{available:0,paid:0,pending:0,entries:[]}]}}));setDetails(Object.fromEntries(pairs));setError("")}catch(e){setError(errorMessage(e))}};
