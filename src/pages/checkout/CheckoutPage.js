...
const failedStatuses = ["refunded", "charged_back", "rejected", "failed", "overdue", "deleted", "canceled", "cancelled", "expired"];
...
export default function CheckoutPage() {
  ...
  const [payerAddress, setPayerAddress] = useState({ zipCode: "", streetName: "", streetNumber: "", neighborhood: "", city: "", state: "" });
  ...
  useEffect(() => {
    if (!payerDocument && user?.cpf) setPayerDocument(String(user.cpf));
  ...
  const paymentProvider = String(catalog?.payment_config?.provider || "").toLowerCase();
  const paymentProviderName = paymentProvider === "asaas" ? "Asaas" : paymentProvider === "mercadopago" ? "Mercado Pago" : "provedor de pagamento";
  const asaasPrimary = paymentProvider === "asaas";
  const pixAvailable = paymentAvailable && methods.includes("pix");
  const cardAvailable = paymentAvailable && methods.includes("card") && (asaasPrimary || Boolean(catalog?.payment_config?.public_key));
  const boletoAvailable = paymentAvailable && methods.includes("boleto");
  const boletoPayerComplete = payerDocument.replace(/\D/g, "").length === 11
    && payerAddress.zipCode.replace(/\D/g, "").length === 8
    && payerAddress.streetName.trim()
    && payerAddress.neighborhood.trim()
    && payerAddress.city.trim()
    && payerAddress.state.trim().length === 2;
  ...
  const payload = (paymentMethod) => ({
    ...
    payer_cpf_cnpj: asaasPrimary ? payerDocument : undefined,
  ...
  const chooseMethod = (nextMethod) => {
    ...
    const methodLabel = nextMethod === "pix" ? "PIX" : nextMethod === "boleto" ? "Boleto" : "Cartão";
    setMethod(nextMethod); writeCheckoutRecovery(slug, { selection, orderPublicId: null, couponCode: coupon?.code || null, paymentMethod: nextMethod }); trackCheckout("payment_method_selected", { label: `${methodLabel} selecionado`, target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(payableTotal.toFixed(2)), payment_method: nextMethod, provider: paymentProvider } });
  ...
  const checkoutCard = async (cardData = {}) => {
    ...
    paymentSubmissionRef.current = true; paymentAttemptedRef.current = true;
    trackCheckout("payment_attempted", { label: "Pagamento com cartão enviado", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(payableTotal.toFixed(2)), discount_amount: discountAmount, coupon_code: coupon?.code || null, payment_method: "card", provider: paymentProvider, checkout_elapsed_ms: Math.max(0, Date.now() - checkoutOpenedAtRef.current) } });
    setPaying(true); setError("");
    try {
      const checkoutResult = await commerceService.checkout({ ...payload("card"), ...cardData });
      setResult(checkoutResult);
      safeSetSessionJson(paymentStorageKey, checkoutResult);
      const redirectUrl = String(checkoutResult?.payment_redirect_url || "").trim();
      if (asaasPrimary && redirectUrl) window.location.assign(redirectUrl);
    }
    catch (err) { const reconciled = await refreshAvailabilityAfterCheckoutConflict(err, "card"); trackCheckout("payment_attempt_failed", { label: "Falha ao processar cartão", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(payableTotal.toFixed(2)), payment_method: "card", provider: paymentProvider, outcome: "error", status: Number(err?.status || err?.response?.status || 0), inventory_reconciled: reconciled } }); if (!reconciled) setError(checkoutSubmissionErrorMessage(err, "Não foi possível processar o cartão.")); }
    finally { paymentSubmissionRef.current = false; setPaying(false); }
  };
  const checkoutBoleto = async () => {
    if (paymentSubmissionRef.current || paying || applyingCoupon || !ensurePaymentAvailable("boleto")) return;
    paymentSubmissionRef.current = true; paymentAttemptedRef.current = true;
    trackCheckout("payment_attempted", { label: "Boleto solicitado", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(payableTotal.toFixed(2)), discount_amount: discountAmount, coupon_code: coupon?.code || null, payment_method: "boleto", provider: paymentProvider, checkout_elapsed_ms: Math.max(0, Date.now() - checkoutOpenedAtRef.current) } });
    setPaying(true); setError("");
    try {
      const checkoutResult = await commerceService.checkout(payload("boleto"));
      setResult(checkoutResult);
      safeSetSessionJson(paymentStorageKey, checkoutResult);
    } catch (err) {
      const reconciled = await refreshAvailabilityAfterCheckoutConflict(err, "boleto");
      trackCheckout("payment_attempt_failed", { label: "Falha ao gerar boleto", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(payableTotal.toFixed(2)), payment_method: "boleto", provider: paymentProvider, outcome: "error", status: Number(err?.status || err?.response?.status || 0), inventory_reconciled: reconciled } });
      if (!reconciled) setError(checkoutSubmissionErrorMessage(err, "Não foi possível gerar o boleto."));
    } finally {
      paymentSubmissionRef.current = false;
      setPaying(false);
    }
  ...
  const copyBoleto = async () => {
    const field = String(result?.boleto?.identification_field || result?.payment?.provider_payload?._boleto?.identificationField || "").trim();
    if (!field) return;
    const copied = await copyText(field);
    setError(copied ? "" : "Não foi possível copiar a linha digitável automaticamente.");
    trackCheckout("boleto_identification_copied", { target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), payment_method: "boleto", provider: paymentProvider, outcome: copied ? "success" : "error" } });
  };
  ...
  const handleMobilePaymentCta = () => {
    ...
    const startsBoletoPayment = method === "boleto" && boletoAvailable;
    const startsAsaasCard = method === "card" && cardAvailable && asaasPrimary;
    const action = startsPixPayment || startsBoletoPayment || startsAsaasCard ? "start_payment" : "focus_payment_form";
    trackCheckout("checkout_mobile_payment_cta_clicked", { label: startsPixPayment ? "Gerar PIX pelo atalho mobile" : startsBoletoPayment ? "Gerar boleto pelo atalho mobile" : startsAsaasCard ? "Abrir pagamento seguro no Asaas" : "Atalho mobile para cartão", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(payableTotal.toFixed(2)), payment_method: method, provider: paymentProvider, action } });
    if (startsPixPayment) {
      checkoutPix();
      return;
    }
    if (startsBoletoPayment) {
      checkoutBoleto();
      return;
    }
    if (startsAsaasCard) {
  ...
  return <div className="cut-checkout-page" data-telemetry-screen="Checkout">
    ...
    <Container className="cut-checkout-container">
      ...
      <div className="cut-checkout-layout"><main className="cut-checkout-main" aria-busy={paying || applyingCoupon || syncingNow}>
        {error && <Alert variant="danger">{error}</Alert>}
        {(paying || applyingCoupon || syncingNow) && <div className="cut-checkout-inline-processing"><ProcessingIndicatorComponent fullscreen={false} label={applyingCoupon ? "Atualizando valores" : syncingNow ? "Verificando pagamento" : "Processando sua solicitação"} /></div>}
