...
export default function CheckoutPage() {
      ...
      commerceService.order(recovery.orderPublicId)
        .then((order) => {
          ...
          trackCheckout("checkout_recovered", {
            ...
            target: slug,
            metadata: { order_status: order?.status || "unknown", fulfillment_status: order?.metadata?.fulfillment_status || "pending" },
          });
        })
        .catch((err) => {
  ...
  useEffect(() => {
    ...
    const reconcile = (chosenItems = [], currentItems = [], limit) => chosenItems
      .map((chosen) => {
  ...
  useEffect(() => {
    ...
    trackCheckout("checkout_opened", {
      ...
      metadata: {
        event_id: Number(catalog.event.id), amount: Number(total.toFixed(2)),
        ticket_quantity: lines.filter((line) => line.kind === "ticket").reduce((sum, line) => sum + line.quantity, 0),
        item_quantity: lines.filter((line) => line.kind === "item").reduce((sum, line) => sum + line.quantity, 0), available_methods: methods,
      },
    });
  ...
  useEffect(() => {
    ...
    trackCheckout("checkout_addon_offer_viewed", {
      label: "Oferta opcional de adicionais exibida", target: slug,
      metadata: { event_id: Number(catalog?.event?.id || 0), current_amount: Number(total.toFixed(2)), ...summarizeCheckoutAddOnOffer(availableAddOns) },
    });
  ...
  useEffect(() => {
    ...
    trackCheckout("pix_payment_ready", {
      label: "PIX pronto para pagamento", target: slug,
      metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(result?.order?.total || total || 0), payment_method: "pix", outcome: "ready" },
    });
  ...
  useEffect(() => {
    ...
    trackCheckout(type, {
      ...
      target: slug,
      metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(result?.order?.total || total || 0), payment_method: method, order_status: orderStatus, fulfillment_status: fulfillmentStatus || "pending", outcome: fulfilled || approved ? "success" : failed ? "error" : "pending" },
    });
  ...
  const syncCurrentPayment = async ({ manual = false } = {}) => {
    ...
    try {
      ...
      if (err?.status === 403 || err?.status === 404) { safeRemoveSessionItem(paymentStorageKey); clearCheckoutRecovery(slug); }
  ...
  useEffect(() => {
    ...
    const sync = async (source = "timer") => {
      ...
      syncing = true;
      const startedAt = Date.now(); const currentAttempt = attempt + 1; let rateLimited = false;
      try {
        const order = await requestPaymentSync(publicId);
        if (!active) return;
        const payments = Array.isArray(order?.payments) ? order.payments : [];
        ...
        if (err?.status === 403 || err?.status === 404) { safeRemoveSessionItem(paymentStorageKey); clearCheckoutRecovery(slug); active = false; }
        if (err?.status && err.status !== 429) setError(err?.message || "Não foi possível atualizar o status do pagamento.");
        ...
        attempt += 1; syncing = false; if (active) scheduleNext({ rateLimited });
      }
    };
  ...
  const addOptionalItem = (item) => {
    ...
    setSelection(nextSelection); safeSetSessionJson(checkoutStorageKey, nextSelection); writeCheckoutRecovery(slug, { selection: nextSelection, orderPublicId: null });
  ...
  const updateItemQuantity = (item, nextQuantity) => {
    if (result || !item?.id) return;
    const quantity = resolveCheckoutQuantity(item, Number(nextQuantity || 0), checkoutQuantityLimit("item"));
  ...
  const payload = (paymentMethod) => ({ event_id: catalog?.event?.id, payment_method: paymentMethod, tickets: (selection?.tickets || []).map((item) => ({ id: Number(item.id), quantity: Number(item.quantity) })), items: (selection?.items || []).map((item) => ({ id: Number(item.id), quantity: Number(item.quantity) })) });
  const ensurePaymentAvailable = (requestedMethod) => {
    if (paymentAvailable && methods.includes(requestedMethod)) return true;
    setError(catalog?.payment_config?.message || "As vendas deste evento ainda não estão habilitadas."); return false;
  };
  const refreshAvailabilityAfterCheckoutConflict = async (err, paymentMethod) => {
    ...
    try {
      ...
      trackCheckout("checkout_inventory_conflict_reconciled", { label: "Disponibilidade atualizada após conflito no checkout", target: slug, metadata: { event_id: Number(freshCatalog?.event?.id || catalog?.event?.id || 0), amount: Number(total.toFixed(2)), payment_method: paymentMethod, status } });
      return true;
  ...
  const chooseMethod = (nextMethod) => {
    ...
    if (paymentPending) {
      trackCheckout("payment_method_change_blocked", { label: "Troca de método bloqueada durante pagamento pendente", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(result?.order?.total || total || 0), previous_payment_method: method, requested_payment_method: nextMethod, order_status: orderStatus || "pending" } });
      setError("Há um pagamento em andamento para esta compra. Aguarde a confirmação ou verifique o status antes de escolher outra forma de pagamento."); return;
    }
    setMethod(nextMethod);
    trackCheckout("payment_method_selected", { label: nextMethod === "pix" ? "PIX selecionado" : "Cartão selecionado", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(total.toFixed(2)), payment_method: nextMethod } });
    if (!approved) { setResult(null); safeRemoveSessionItem(paymentStorageKey); writeCheckoutRecovery(slug, { selection, orderPublicId: null }); }
  };
  ...
  const recoverFailedPayment = (nextMethod = method) => {
    trackCheckout("payment_recovery_selected", { label: nextMethod === method ? "Tentar pagamento novamente" : `Trocar para ${nextMethod === "pix" ? "PIX" : "cartão"}`, target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(result?.order?.total || total || 0), previous_payment_method: method, payment_method: nextMethod, previous_status: orderStatus || "failed", failure_reason: failedPaymentGuidance.reason } });
    setMethod(nextMethod); setResult(null); setError(""); safeRemoveSessionItem(paymentStorageKey); writeCheckoutRecovery(slug, { selection, orderPublicId: null });
  };
  const copyPix = async () => {
    const pixCode = result?.payment?.qr_code; if (!pixCode) return;
    const copied = await copyText(pixCode);
    if (!copied) { setPixCopyStatus("error"); setError("Não foi possível copiar automaticamente neste navegador. Toque e segure o código PIX exibido nesta tela para selecionar e copiar."); trackCheckout("pix_code_copy_failed", { label: "Falha ao copiar código PIX", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(result?.order?.total || total || 0), payment_method: "pix", outcome: "error" } }); return; }
    setPixCopyStatus("copied"); setError(""); trackCheckout("pix_code_copied", { label: "Código PIX copiado", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(result?.order?.total || total || 0), payment_method: "pix", outcome: "success" } }); window.setTimeout(() => setPixCopyStatus("idle"), 2500);
  };
  ...
  return <div className="cut-checkout-page" data-telemetry-screen="Checkout">
    ...
    <Container className="cut-checkout-container">
      ...
      <div className="cut-checkout-layout"><main className="cut-checkout-main">
        ...
        {fulfilled ? <section className="cut-checkout-success"><div className="cut-checkout-success__icon"><i className="fa-solid fa-check" /></div><span>Pagamento aprovado</span><h2>Compra confirmada!</h2><p>Seu pagamento foi reconhecido e seus ingressos já estão liberados.</p><div className="cut-checkout-receipt"><div><span>Pedido</span><strong>#{result.order?.id}</strong></div><div><span>Total pago</span><strong>{money(result.order?.total)}</strong></div></div>{(result.order?.items || []).map((item) => <div className="cut-checkout-purchased" key={item.id}><div><small>{item.type === "ticket" ? "INGRESSO" : "ITEM"}</small><strong>{item.name}</strong><span>{item.quantity} × {money(item.unit_price)}</span></div><i className="fa-solid fa-circle-check" /></div>)}<Button as={Link} to="/passes" className="cut-checkout-primary mt-3">Ver meus ingressos</Button></section> : <>
          <section className="cut-checkout-section" data-telemetry-context="Forma de pagamento"><div className="cut-checkout-section__head"><div className="cut-checkout-step">1</div><div><h2>Forma de pagamento</h2><p>{paymentPending ? "Há um pagamento em andamento. A forma de pagamento fica protegida até a confirmação ou falha desta tentativa." : "Escolha como deseja pagar."}</p></div></div><div className="cut-payment-methods">{pixAvailable && <button type="button" data-track="Selecionar PIX" className={method === "pix" ? "is-active" : ""} onClick={() => chooseMethod("pix")} disabled={paymentPending && method !== "pix"} aria-disabled={paymentPending && method !== "pix"}><i className="fa-brands fa-pix" /><div><strong>PIX</strong><span>Aprovação rápida</span></div><i className="fa-solid fa-circle-check" /></button>}{cardAvailable && <button type="button" data-track="Selecionar cartão" className={method === "card" ? "is-active" : ""} onClick={() => chooseMethod("card")} disabled={paymentPending && method !== "card"} aria-disabled={paymentPending && method !== "card"}><i className="fa-regular fa-credit-card" /><div><strong>Cartão de crédito</strong><span>Pagamento protegido</span></div><i className="fa-solid fa-circle-check" /></button>}</div></section>
