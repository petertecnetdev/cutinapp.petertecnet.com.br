...
import { clearCheckoutPaymentRecovery, readCheckoutPaymentRecovery, writeCheckoutPaymentRecovery } from "../../utils/checkoutPaymentRecovery";
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    commerceService.catalog(slug)
      .then((response) => {
        if (!active) return;
        setCatalog(response);
        const nextMethods = Array.isArray(response?.payment_config?.methods) ? response.payment_config.methods : [];
        if (nextMethods.length && !nextMethods.includes("pix")) setMethod(nextMethods[0]);
      })
  ...
  useEffect(() => {
    ...
    const reconcile = (chosenItems = [], currentItems = [], limit = 20) => chosenItems.map((chosen) => {
  ...
  useEffect(() => {
    ...
    if (approved) {
      const recovery = readCheckoutPaymentRecovery(slug);
      if (recovery) {
        trackCheckout("checkout_failed_payment_recovered", {
          label: "Pagamento recuperado após recusa",
          target: slug,
          metadata: {
            event_id: Number(catalog?.event?.id || 0),
            recovered_gmv: Number(result?.order?.total || recovery.amount || total || 0),
            failure_reason: recovery.reason,
            previous_payment_method: recovery.fromMethod,
            payment_method: method,
            intended_recovery_method: recovery.toMethod,
            recovery_duration_ms: Math.max(0, Date.now() - recovery.startedAt),
            outcome: "success",
          },
        });
        clearCheckoutPaymentRecovery(slug);
      }
    }
    ...
    const sync = async (source = "timer") => {
      ...
      try {
        ...
        trackCheckout("payment_sync_cycle", {
          ...
          target: slug,
          metadata: { source, attempt: currentAttempt, duration_ms: Date.now() - startedAt, order_status: order?.status || "unknown", fulfillment_status: order?.metadata?.fulfillment_status || "pending", outcome: "success" },
        });
      } catch (err) {
        ...
        trackCheckout("payment_sync_cycle", {
          ...
          target: slug,
          metadata: { source, attempt: currentAttempt, duration_ms: Date.now() - startedAt, status: Number(err?.status || 0), outcome: rateLimited ? "rate_limited" : "error" },
        });
      } finally {
        attempt += 1;
        syncing = false;
  ...
  const recoverFailedPayment = (nextMethod = method) => {
    writeCheckoutPaymentRecovery({ slug, reason: failedPaymentGuidance.reason, fromMethod: method, toMethod: nextMethod, amount: Number(result?.order?.total || total || 0) });
    trackCheckout("payment_recovery_selected", {
      ...
      target: slug,
      metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(result?.order?.total || total || 0), previous_payment_method: method, payment_method: nextMethod, previous_status: orderStatus || "failed", failure_reason: failedPaymentGuidance.reason },
    });
    setMethod(nextMethod);
    setResult(null);
    setError("");
  ...
  const copyPix = async () => {
    ...
    if (!copied) {
      ...
      trackCheckout("pix_code_copy_failed", { label: "Falha ao copiar código PIX", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(result?.order?.total || total || 0), payment_method: "pix", outcome: "error" } });
      return;
    }
    setPixCopyStatus("copied");
    setError("");
    trackCheckout("pix_code_copied", { label: "Código PIX copiado", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(result?.order?.total || total || 0), payment_method: "pix", outcome: "success" } });
