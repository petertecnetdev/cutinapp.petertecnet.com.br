...
export default function CheckoutPage() {
  ...
  const handleMobilePaymentCta = () => {
    ...
    trackCheckout("checkout_mobile_payment_cta_clicked", { label: startsPixPayment ? "Gerar PIX pelo atalho mobile" : "Atalho mobile para cartão", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(payableTotal.toFixed(2)), payment_method: method, action: startsPixPayment ? "start_payment" : "focus_payment_form" } });
    if (startsPixPayment) {
      checkoutPix();
      return;
    }
