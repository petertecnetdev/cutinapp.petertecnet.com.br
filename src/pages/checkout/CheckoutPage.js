  ...
  return <div className="cut-checkout-page" data-telemetry-screen="Checkout">
    ...
    <Container className="cut-checkout-container">
      ...
      <aside className="cut-checkout-summary">
        ...
        <div className="cut-checkout-summary__lines">
          {lines.map((line) => <div key={`${line.kind}-${line.id}`}><div><small>{line.kind === "ticket" ? "Ingresso" : "Item"}</small><strong>{line.name}</strong>{!result ? <div className="cut-checkout-line-controls" aria-label={`Quantidade de ${line.name}`}><Button type="button" variant="outline-light" size="sm" onClick={() => updateLineQuantity(line, line.quantity - 1)} disabled={applyingCoupon} aria-label={`Diminuir quantidade de ${line.name}`}><i className="fa-solid fa-minus" /></Button><span aria-live="polite">{line.quantity}</span><Button type="button" variant="outline-light" size="sm" onClick={() => updateLineQuantity(line, line.quantity + 1)} disabled={applyingCoupon || line.quantity >= resolveCheckoutQuantity(line, checkoutQuantityLimit(line.kind), checkoutQuantityLimit(line.kind))} aria-label={`Aumentar quantidade de ${line.name}`}><i className="fa-solid fa-plus" /></Button><Button type="button" variant="link" size="sm" className="cut-checkout-line-remove" onClick={() => updateLineQuantity(line, 0)} disabled={applyingCoupon} aria-label={`Remover ${line.name} do carrinho`}><i className="fa-regular fa-trash-can" /></Button></div> : <span>Qtd. {line.quantity}</span>}</div><strong>{money(Number(line.price) * line.quantity)}</strong></div>)}
        </div>
        {coupon && !result && <div className="cut-checkout-summary__discount"><span><i className="fa-solid fa-tag" /> Cupom {coupon.code}</span><strong>- {money(discountAmount)}</strong></div>}
        <div className="cut-checkout-summary__total"><span>Total</span><strong>{money(result?.order?.total ?? payableTotal)}</strong></div>
        <div className="cut-checkout-summary__security"><i className="fa-solid fa-shield-halved" /><span>{isFreeOrder ? "Pedido gratuito sem cobrança, com emissão normal do ingresso." : `Pagamento processado com segurança pelo ${paymentProviderName}.`}</span></div>
      </aside>
    </div>
  </Container>
  {!result && paymentAvailable && payableTotal > 0 && <div className="cut-checkout-mobile-paybar" role="region" aria-label="Atalho para pagamento">
    <div><small>{method === "pix" ? "PIX selecionado" : method === "boleto" ? "Boleto selecionado" : "Cartão selecionado"}</small><strong>{money(payableTotal)}</strong></div>
    <Button type="button" className="cut-checkout-primary" onClick={handleMobilePaymentCta} disabled={paying || applyingCoupon}>
      {applyingCoupon ? "Atualizando total..." : paying ? "Processando..." : method === "pix" ? "Gerar PIX agora" : method === "boleto" ? "Gerar boleto" : asaasPrimary ? "Continuar no Asaas" : "Ir para cartão"}
    </Button>
  </div>}
</div>;
}
