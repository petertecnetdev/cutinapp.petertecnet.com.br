...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    trackCheckout(type, { label: fulfilled ? "Ingresso emitido" : approved ? "Pagamento aprovado" : failed ? "Pagamento não concluído" : "Pagamento aguardando confirmação", target: slug, metadata: { event_id: Number(catalog?.event?.id || 0), amount: Number(result?.order?.total || payableTotal || 0), payment_method: method, order_status: orderStatus, fulfillment_status: fulfillmentStatus || "pending", outcome: fulfilled || approved ? "success" : failed ? "error" : "pending" } });
  }, [approved, catalog?.event?.id, failed, fulfilled, fulfillmentStatus, method, orderStatus, payableTotal, result?.order?.public_id, result?.order?.total, slug]);
  ...
  useEffect(() => {
    const publicId = result?.order?.public_id; const status = result?.order?.status || result?.payment?.status; const fulfillment = result?.order?.metadata?.fulfillment_status; const terminal = failedStatuses.includes(status) || (status === "paid" && fulfillment === "completed");
  ...
  const addOptionalItem = (item) => {
    ...
    setSelection(nextSelection); setCoupon(null); setCouponError(""); safeSetSessionJson(checkoutStorageKey, nextSelection); writeCheckoutRecovery(slug, { selection: nextSelection, orderPublicId: null });
  ...
  const checkoutPix = async () => {
    if (paymentSubmissionRef.current || paying || !ensurePaymentAvailable("pix")) return;
  ...
  const checkoutCard = async (cardData) => {
    if (paymentSubmissionRef.current || paying || !ensurePaymentAvailable("card")) return;
    paymentSubmissionRef.current = true; paymentAttemptedRef.current = true;
