...
const notificationIcon = (type = "") => {
  ...
  if (type === "checkout_recovery") return "fa-brands fa-pix";
  return "fa-solid fa-earth-americas";
};

const recoveryCtaLabel = (item) => {
  if (item?.type !== "checkout_recovery" || item?.metadata?.recovery_action !== "resume_pix") return "";
  return String(item?.metadata?.recovery_cta_label || "Retomar pagamento PIX").trim().slice(0, 80);
};

const trackRecoveryCtaClick = (item) => {
  const label = recoveryCtaLabel(item);
  if (!label) return;
  ...
  try {
    window.PeterTecnetTelemetry?.track?.("checkout_recovery_notification_cta_clicked", {
      ...
      metadata: {
        order_public_id: String(item?.reference_id || ""),
        recovery_source: "in_app",
        recovery_entrypoint: "notifications_page",
        recovery_action: "resume_pix",
        recovery_experiment: item?.metadata?.recovery_experiment || null,
        recovery_timing_minutes: Number(item?.metadata?.recovery_timing_minutes || 0) || null,
      },
    });
  } catch (_) {
