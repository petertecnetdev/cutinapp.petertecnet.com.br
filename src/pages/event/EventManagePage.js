  ...
  return (
    <div className="cut-app-page">
      ...
      <Container className="cut-page-container py-4 py-lg-5">
        <div className="cut-page-heading"><div><span className="cut-eyebrow">Área do produtor</span><h1>Meus eventos</h1><p>Controle rascunho, ingressos, comunicação, publicação, participantes e portaria sem misturar as etapas.</p></div><Button onClick={() => navigate("/event/create")}><i className="fa-solid fa-plus me-2" />Novo evento</Button></div>
        ...
        {!loading && events.length === 0 ? (
          ...
          <Row className="g-4">{events.map((event) => <Col lg={6} key={event.id}><Card className="cut-panel h-100"><Card.Body className="p-4">
            ...
            <div className="cut-info-box mt-3"><strong>{event.tickets_count || 0} lote(s) de ingresso</strong><span>{event.is_published ? "Página pública ativa. Use a Central de avisos para comunicar mudanças ao público." : "Finalize os ingressos e a preparação do evento antes de publicar."}</span></div>
            <div className="cut-card-actions mt-4">
              <Button onClick={() => navigate(`/event/edit/${event.id}`)}>Editar</Button>
              <Button variant="outline-info" onClick={() => navigate(`/event/${event.id}/announcements`)}><i className="fa-solid fa-bullhorn me-2" />Avisos</Button>
