...
export default function EventLineupPage() {
  ...
  const add = async (e) => {
    ...
    try { const response = await cutinappService.attachArtist(eventId, { ...form, artist_id: Number(form.artist_id), sort_order: Number(form.sort_order || 0) }); setArtists(response.artists || []); setSuccess(response.message || "Atração adicionada ao line-up."); setForm({ artist_id: "", participation_type: "show", description: "", scheduled_at: "", stage: "", sort_order: 0, is_headliner: false }); }
    catch (err) { setError(err?.message || "Não foi possível adicionar a atração."); }
    finally { setBusy(false); }
  };

  const remove = async (artistId) => {
    setBusy(true); setError("");
    try { await cutinappService.detachArtist(eventId, artistId); await load(); setSuccess("Atração removida do line-up."); }
    catch (err) { setError(err?.message || "Não foi possível remover a atração."); }
    finally { setBusy(false); }
  };
