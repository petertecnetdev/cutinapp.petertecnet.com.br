...
export default function ArtistViewPage() {
  ...
  return <div className="cut-app-page"><NavlogComponent />
    <section className="cut-profile-hero" style={artist.cover ? { backgroundImage: `linear-gradient(180deg,rgba(2,8,13,.1),rgba(2,8,13,.95)),url(${mediaUrl(artist.cover)})` } : undefined}>
      <Container className="cut-page-container"><div className="cut-profile-hero__content"><div className="cut-profile-avatar">{artist.photo ? <img src={mediaUrl(artist.photo)} alt={artist.stage_name} /> : <span>{artist.stage_name?.slice(0,2).toUpperCase()}</span>}</div><div><div className="d-flex align-items-center gap-2 flex-wrap"><span className="cut-eyebrow">{TYPE_LABELS[artist.artist_type] || "Artista Cutinapp"}</span>{artist.verification_status === "verified" && <Badge bg="success"><i className="fa-solid fa-circle-check me-1" />Verificado</Badge>}{artist.verification_status === "account_linked" && <Badge bg="info">Conta vinculada</Badge>}</div><h1>{artist.stage_name}</h1><p>{artist.city ? `${artist.city}${artist.uf ? ` - ${artist.uf}` : ""}` : ""}</p>{publicReference && <p className="mb-2"><i className="fa-solid fa-handshake-angle me-2" />Apresentado na Cutinapp por <strong>{artist.origin_label}</strong></p>}<div className="cut-social-stats"><span>{artist.followers_count || 0} seguidores</span><span>{upcoming.length} próximos eventos</span><span>{upcoming.length + past.length} eventos na Cutinapp</span>{relatedProductions.length > 0 && <span>{relatedProductions.length} produç{relatedProductions.length === 1 ? "ão relacionada" : "ões relacionadas"}</span>}{isGroup && <span>{currentMembers.length} integrante(s)</span>}</div><div className="cut-card-actions mt-3"><Button onClick={toggleFollow} disabled={busy}>{artist.is_following ? "Seguindo" : isGroup ? "Seguir formação" : "Seguir artista"}</Button>{instagramHref && <Button as="a" href={instagramHref} target="_blank" rel="noopener noreferrer" variant="outline-light">Instagram</Button>}{spotifyHref && <Button as="a" href={spotifyHref} target="_blank" rel="noopener noreferrer" variant="outline-light">Spotify</Button>}</div></div></div></Container>
    </section>
