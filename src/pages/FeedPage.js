...
import SocialThread from "../components/social/SocialThread";
...
export default function FeedPage() {
  ...
  const load = useCallback(async (nextPage = 1) => {
    ...
    try {
      ...
      setError(err?.message || "Não foi possível montar suas recomendações agora.");
    } finally {
      setLoading(false);
  ...
  return <div className="cut-app-page"><NavlogComponent />
    <Container className="cut-page-container py-4 py-lg-5">
      <div className="cut-page-heading"><div><span className="cut-eyebrow">Comunidade aberta</span><h1>Feed geral Cutinapp</h1><p>Converse com todo mundo: pergunte o que tem de rolê hoje, combine encontros, fale de eventos e compartilhe descobertas de qualquer cidade.</p>{user && context.preferred_city && <span className="cut-feed-context"><i className="fa-solid fa-location-dot" /> Seus eventos priorizam {context.preferred_city}{context.preferred_uf ? ` - ${context.preferred_uf}` : ""}</span>}</div><div className="cut-card-actions"><Button onClick={() => navigate("/event")}>Explorar eventos</Button>{!user && <Button variant="outline-light" onClick={() => navigate("/login", { state: { from: "/feed" } })}>Entrar para publicar</Button>}</div></div>
      {error && <Alert variant="danger">{error}</Alert>}

      <SocialThread
        scopeType="global"
        eyebrow="Agora na Cutinapp"
        title="O que está rolando?"
        description="Tudo aqui é público. Publique uma pergunta, uma indicação, uma experiência ou qualquer assunto relacionado a rolês e eventos."
      />

      {!user ? <Card className="cut-feed-public-cta mt-5"><Card.Body><div><span className="cut-eyebrow">Descoberta</span><h2>Quer um feed de eventos feito para você?</h2><p>Entre na Cutinapp para combinar o feed público com eventos da sua cidade, produções e artistas que você acompanha e ingressos que já possui.</p></div><div className="cut-card-actions"><Button onClick={() => navigate("/login", { state: { from: "/feed" } })}>Entrar</Button><Button variant="outline-light" onClick={() => navigate("/event")}>Ver eventos</Button></div></Card.Body></Card> : loading ? <Row className="g-4 mt-4" aria-busy="true">{Array.from({ length: 6 }).map((_, index) => <Col md={6} xl={4} key={index}><SkeletonCard /></Col>)}</Row> : <section className="mt-5">
