...
export default function ProductionFinancePage() {
  ...
  const loadFinance = async (id, active = () => true) => {
  ...
  const requestPayout = async () => {
    ...
    try {
      ...
      setSuccess(response?.message || "Repasse solicitado com sucesso.");
    ...
    <Container className="cut-page-container py-4 py-lg-5">
      <div className="cut-page-heading">
        <div>
          ...
          <h1>Recebimentos</h1>
          <p>Vendas novas usam split automático quando a produção está conectada ao Mercado Pago. Valores recebidos anteriormente pela plataforma ficam disponíveis para repasse.</p>
        </div>
        <Button variant="outline-light" onClick={() => navigate("/production/mine")}>Minhas produções</Button>
      </div>

      {connectedNotice && <Alert variant="success">Mercado Pago conectado com sucesso. As próximas vendas já podem usar split automático.</Alert>}
      ...
      {productionId && <>
            ...
            <Card className="cut-production-card h-100">
              <Card.Body className="p-4">
                ...
                <div className="d-flex align-items-center gap-2 mb-3">
                  <Badge bg={connected ? "success" : "warning"}>{connected ? "Split automático ativo" : "Conta não conectada"}</Badge>
          ...
          <Col lg={7}>
            <Card className="cut-production-card h-100">
              <Card.Body className="p-4">
                ...
                <Alert variant={connected ? "success" : "info"} className="mt-4 mb-0">
                  {connected ? "As novas vendas são direcionadas pelo fluxo de split automático do Mercado Pago." : "Enquanto a conta não estiver conectada, vendas compatíveis podem ser recebidas pela plataforma e registradas como saldo a repassar."}
                </Alert>
              </Card.Body>
            </Card>
          </Col>
        ...
        <Card className="cut-production-card mt-4">
          <Card.Body className="p-4">
            <div className="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
              <div><span className="cut-eyebrow">Saldo de repasse</span><h2 className="mt-2 mb-1">Valores recebidos pela plataforma</h2><p className="text-secondary mb-0">Este saldo reúne somente vendas feitas no modo de recebimento pela plataforma.</p></div>
              <Badge bg={payouts?.current_settlement_mode === "automatic_split" ? "success" : "warning"}>{payouts?.current_settlement_mode === "automatic_split" ? "Novas vendas: split automático" : "Novas vendas: plataforma"}</Badge>
            </div>
            <Row className="g-3 mb-4">
              <Col md={3}><div className="cut-ticket-option h-100"><div><span className="cut-ticket-kicker">Crédito acumulado</span><strong>{money(payouts?.platform_collected_credit)}</strong></div></div></Col>
              <Col md={3}><div className="cut-ticket-option h-100"><div><span className="cut-ticket-kicker">Disponível</span><strong>{money(availableForPayout)}</strong></div></div></Col>
              <Col md={3}><div className="cut-ticket-option h-100"><div><span className="cut-ticket-kicker">Em processamento</span><strong>{money(payouts?.payout_pending)}</strong></div></div></Col>
              <Col md={3}><div className="cut-ticket-option h-100"><div><span className="cut-ticket-kicker">Já repassado</span><strong>{money(payouts?.payout_paid)}</strong></div></div></Col>
            </Row>

            {availableForPayout > 0 && !connected && <Alert variant="warning">Conecte a conta Mercado Pago da produção para habilitar a solicitação de repasse deste saldo.</Alert>}
            {availableForPayout > 0 && connected && <div className="d-flex flex-column flex-md-row gap-2 align-items-md-end mb-4">
              <Form.Group className="flex-grow-1"><Form.Label>Valor do repasse</Form.Label><Form.Control type="number" min="0.01" step="0.01" max={availableForPayout} value={payoutAmount} onChange={(event) => setPayoutAmount(event.target.value)} /></Form.Group>
              <Button onClick={requestPayout} disabled={requesting}>Solicitar repasse</Button>
            </div>}
            {availableForPayout <= 0 && <Alert variant="secondary">Não há saldo de vendas recebidas pela plataforma aguardando repasse.</Alert>}

            {(payouts?.history || []).length > 0 && <div className="table-responsive mt-4">
              <Table variant="dark" hover className="align-middle mb-0">
                <thead><tr><th>Referência</th><th>Solicitado em</th><th>Valor</th><th>Status</th><th></th></tr></thead>
                <tbody>{payouts.history.map((row) => { const status = payoutStatus[row.status] || { label: row.status, bg: "secondary" }; return <tr key={row.id}><td><small>{row.reference}</small></td><td>{dateTime(row.requested_at)}</td><td><strong>{money(row.amount)}</strong></td><td><Badge bg={status.bg}>{status.label}</Badge></td><td className="text-end">{row.status === "pending" && <Button size="sm" variant="outline-light" onClick={() => cancelPayout(row.id)} disabled={requesting}>Cancelar</Button>}</td></tr>; })}</tbody>
              </Table>
            </div>}
          </Card.Body>
        </Card>
