...
import { canonicalOrganizationType, normalizeOrganizationRoles } from "../../domain/organizations/organizationTaxonomy";
...
const experienceKeys = ["type", "roles", "city_id", "city", "uf", "cep", "address", "address_number", "neighborhood", "address_complement", "address_reference", "formatted_address", "latitude", "longitude", "place_id", "google_maps_url", "location_public"];
...
export default function ProductionUpdatePage() {
  ...
  useEffect(() => {
    ...
    cutinappService.productionWorkspace(id).then(({ organization: production }) => {
      if (!active) return;
      const type = canonicalOrganizationType(production.type);
      setForm({
        name: production.name || "",
        fantasy: production.fantasy || "",
        type,
        roles: normalizeOrganizationRoles(production.roles, type),
    ...
    }).catch((err) => active && setError(err?.message || "Não foi possível carregar a organização."))
  ...
  const canSave = useMemo(() => Boolean(form?.name?.trim()) && Boolean(form?.roles?.length) && !saving, [form, saving]);
  ...
  const submit = async (event) => {
    ...
    setError("");
    setFieldErrors({});
    if (!canSave || nameInvalid || cnpjInvalid || locationInvalid) {
    ...
    try {
      ...
      const profile = {};
      experienceKeys.forEach((key) => { profile[key] = form[key] === "" ? null : form[key]; });
      profile.location_public = Boolean(form.location_public);
  ...
  if (loading) return <div className="cut-app-page"><NavlogComponent /><ProcessingIndicatorComponent label="Carregando organização" /></div>;

  return <div className="cut-app-page">
    <NavlogComponent />
    {saving && <ProcessingIndicatorComponent label="Salvando organização" />}
    <Container className="cut-page-container py-4 py-lg-5">
      <div className="cut-page-heading">
        <div>
          <span className="cut-eyebrow">Área do produtor</span>
          <h1>Editar organização</h1>
          <p>Atualize identidade, tipo, atuações e o local que o público poderá encontrar.</p>
        </div>
          ...
          <Col lg={8}>
            <Card className="cut-panel">
              <Card.Body className="p-4 p-lg-5">
                <span className="cut-eyebrow">Identidade</span>
                <h2 className="cut-section-title mt-2">Dados da organização</h2>
                <Row className="g-3">
                ...
                <h2 className="cut-section-title mt-4">Localização</h2>
                <LocationFields value={form} onChange={setForm} showPublicToggle />
                {locationInvalid && <Alert variant="warning" className="mt-3">Selecione a cidade na lista de resultados.</Alert>}
          ...
          <Col lg={4}>
            <Card className="cut-panel">
              <Card.Body className="p-4">
                <span className="cut-eyebrow">Marca</span>
                <h2 className="cut-section-title mt-2">Imagens</h2>
                <Form.Group className="mb-3"><Form.Label>Logo</Form.Label>{logoPreview && <img className="cut-upload-preview cut-upload-preview--logo" src={logoPreview} alt="Logo" />}<Form.Control type="file" accept="image/png,image/jpeg,image/webp" onChange={chooseFile(setLogo, setLogoPreview, "logo")} /></Form.Group>
                <Form.Group><Form.Label>Capa</Form.Label>{bgPreview && <img className="cut-upload-preview" src={bgPreview} alt="Capa" />}<Form.Control type="file" accept="image/png,image/jpeg,image/webp" onChange={chooseFile(setBackground, setBgPreview, "background")} /></Form.Group>
