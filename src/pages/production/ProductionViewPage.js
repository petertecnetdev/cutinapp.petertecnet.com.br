...
export default function ProductionViewPage() {
  ...
  const orderedEvents = useMemo(() => [...events].sort((a, b) => {
    const aTime = new Date(a?.start_date || a?.starts_at || 0).getTime();
    const bTime = new Date(b?.start_date || b?.starts_at || 0).getTime();
  ...
  const upcomingEvents = useMemo(() => {
    ...
    return orderedEvents.filter((event) => {
      const time = new Date(event?.start_date || event?.starts_at || 0).getTime();
      return !Number.isFinite(time) || time >= threshold;
    });
  }, [orderedEvents]);
  const nextEvent = upcomingEvents[0] || orderedEvents[0] || null;
  ...
  const shareProduction = async () => {
    ...
    const message = `Conheça a produção ${production.name} na Cutinapp`;
    try {
      if (navigator.share) {
        await navigator.share({ title: production.name, text: message, url: publicUrl });
        return;
      }
      const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`${message}: ${publicUrl}`)}`;
      window.open(whatsappUrl, "_blank", "noopener,noreferrer");
    } catch (_) {
      // Cancelar o compartilhamento nativo não deve interromper a página.
    }
  };
    ...
    {production && <>
      ...
      <Container className="cut-page-container py-4 py-lg-5">
        ...
        <ProductionNextEventHero
          ...
          allTo={production.slug ? `/agenda/${production.slug}` : ""}
          emptyTitle="Nenhum próximo evento publicado"
          emptyText="Crie ou publique a próxima data para ela ganhar destaque nesta produção."
        />

        <section className="cut-production-section cut-production-agenda-section cut-production-agenda-section--priority"><div className="cut-production-section-head"><div><span className="cut-eyebrow">Agenda</span><h2>Eventos desta produção</h2></div><div className="cut-production-section-actions">{production.slug && <Button variant="outline-light" onClick={() => navigate(`/agenda/${production.slug}`)}><i className="fa-regular fa-calendar me-2" />Agenda pública</Button>}<Button variant="outline-light" onClick={() => navigate(`/event/create?productionId=${production.id}`)}><i className="fa-solid fa-plus me-2" />Novo evento</Button></div></div>{orderedEvents.length === 0 ? <Card className="cut-empty-state"><Card.Body><p>Nenhum evento cadastrado nesta produção ainda.</p></Card.Body></Card> : <div className="cut-production-events-carousel">{orderedEvents.slice(0, eventsVisible).map((event) => <article className="cut-production-event-slide" key={event.id} role="link" tabIndex={0} aria-label={`Abrir evento ${event.title}`} onClick={() => navigate(`/event/${event.slug}`)} onKeyDown={(e) => activateOnKeyboard(e, () => navigate(`/event/${event.slug}`))}><div className="cut-production-event-slide__media"><EventPosterThumbnail image={event.image} title={event.title} alt={event.title} className="cut-production-event-slide__poster" loading="lazy" /></div><div className="cut-production-event-slide__body"><span className="cut-eyebrow">{event.category || "Evento"}</span><h3>{event.title}</h3><p><i className="fa-regular fa-calendar me-2" />{fmt(event.start_date)}</p><p><i className="fa-solid fa-location-dot me-2" />{event.venue || event.city || "Local a definir"}</p></div></article>)}</div>}{eventsVisible < orderedEvents.length && <div className="cut-production-progressive-actions"><Button variant="outline-light" onClick={() => setEventsVisible((value) => value + 8)}>Carregar mais eventos</Button><span>{Math.min(eventsVisible, orderedEvents.length)} de {orderedEvents.length}</span></div>}</section>

        <React.Suspense fallback={<div className="cut-production-section cut-production-async-indicator"><ProcessingIndicatorComponent fullscreen={false} label="Preparando galeria" /></div>}><ProductionGallery
          media={media}
        ...
        <React.Suspense fallback={<div className="cut-production-section cut-production-async-indicator"><ProcessingIndicatorComponent fullscreen={false} label="Preparando publicações" /></div>}><ProductionCommunitySection production={production} isOwner /></React.Suspense>
      </Container>
