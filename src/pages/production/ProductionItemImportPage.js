...
export default function ProductionItemImportPage() {
  ...
  useEffect(() => {
    ...
    cutinappService.productionWorkspace(id)
      .then((workspace) => {
        if (active) setProduction(workspace?.organization || null);
      })
      .catch((err) => {
        if (active) setError(errorMessage(err, "Não foi possível abrir a produção."));
      })
      .finally(() => {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      payload.append("currency", "BRL");

      const response = await cutinappService.previewItemImport(id, payload);
      ...
      setError(errorMessage(err, "Não foi possível analisar as imagens."));
    } finally {
      setAnalyzing(false);
    }
  };
  ...
  return <div className="cut-app-page">
    ...
    <Container className="cut-page-container py-4 py-lg-5">
      <div className="cut-ai-import-head">
        <div>
          <span className="cut-eyebrow">Catálogo inteligente</span>
          <h1>Importar cardápio com IA</h1>
          <p>Envie fotos do cardápio de {production?.name || "seu estabelecimento"}. A Cutinapp identifica os itens e você revisa tudo antes do cadastro.</p>
        </div>
      ...
      <Card className="cut-panel cut-ai-import-upload">
        <Card.Body className="p-4 p-lg-5">
          ...
          <div>
            ...
            <p>Você pode enviar até {MAX_IMAGES} imagens JPG, PNG ou WebP. Fotos nítidas, retas e com o preço visível dão resultados melhores.</p>
          </div>
          <Form.Control
            type="file"
          ...
          <div className="cut-ai-import-upload__actions">
            <Button onClick={analyze} disabled={!images.length || analyzing || saving}>
              {analyzing ? <><span className="spinner-border spinner-border-sm me-2" />Lendo cardápio...</> : <><i className="fa-solid fa-wand-magic-sparkles me-2" />Analisar imagens</>}
            </Button>
            {items.length > 0 && <Button variant="outline-light" onClick={() => { setImages([]); setItems([]); setWarnings([]); }} disabled={saving}>Começar novamente</Button>}
          </div>
        </Card.Body>
      </Card>
            ...
            return <Card className={`cut-panel cut-ai-import-item ${invalid ? "cut-ai-import-item--invalid" : ""}`} key={`${item.name}-${index}`}>
              <Card.Body className="p-4">
                <div className="cut-ai-import-item__top">
                  <div>
                    <Badge bg={item.confidence >= 0.9 ? "success" : item.confidence >= 0.7 ? "warning" : "secondary"}>
                ...
                {item.duplicate && <div className="cut-ai-import-duplicate">
                  ...
                  <span>Já existe <strong>{item.duplicate.name}</strong> por {money(item.duplicate.price)}.</span>
                </div>}
