    ...
    <Container className="cut-page-container py-4 py-lg-5">
      <section className="cut-admin-events-hero">
        <div>
          ...
          <h1>Eventos</h1>
          <p>{total || events.length} evento(s) na Cutinapp. Toque em editar para abrir a administração do evento.</p>
        </div>
      </section>
      ...
      {loading && !events.length ? <div className="text-center py-5"><Spinner /><p className="mt-2">Carregando eventos...</p></div> : <div className="cut-admin-event-list">
        {events.map((event) => {
          ...
          return <article className="cut-admin-event-row" key={event.id}>
