...
describe("fulfilled checkout storage cleanup", () => {
  ...
  test("merges production ticket selections into the same canonical event cart", () => {
    const result = mergeEventCartTickets({ eventId: 77, tickets: [{ id: 1, quantity: 2 }], items: [{ id: 9, quantity: 1 }] }, [
  ...
  test("prefers a newer local cart and repairs a stale tab session snapshot", () => {
    const slug = "evento-multitab"; const cartKey = `cutinapp_checkout_${slug}`;
  ...
  test("prefers shared local cart when cross-tab writes have the same timestamp", () => {
    const slug = "evento-multitab-mesmo-ms"; const cartKey = `cutinapp_checkout_${slug}`;
  ...
  test("shared clear tombstone prevents another tab from resurrecting a stale cart", () => {
    const slug = "evento-limpo-em-outra-aba"; const cartKey = `cutinapp_checkout_${slug}`;
    window.sessionStorage.setItem(cartKey, JSON.stringify({ tickets: [{ id: 8, quantity: 2 }], items: [], savedAt: 1000 }));
  ...
  test("clears draft recovery when the participant empties the cart", () => {
    const slug = "evento-vazio"; writeEventCart(slug, { tickets: [{ id: 5, quantity: 1 }], items: [] });
