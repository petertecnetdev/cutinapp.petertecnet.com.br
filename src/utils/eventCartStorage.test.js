...
describe("fulfilled checkout storage cleanup", () => {
  ...
  test("merges production ticket selections into the same canonical event cart", () => {
    const result = mergeEventCartTickets({ eventId: 77, eventDate: "2026-09-20T20:00:00-03:00", tickets: [{ id: 1, quantity: 2 }], items: [{ id: 9, quantity: 1 }] }, [
      { id: 1, quantity: 3, maxQuantity: 4 }, { id: 2, quantity: 2, maxQuantity: 5 },
    ]);
  ...
  test("never reduces an existing ticket quantity when availability shrinks during an add", () => {
    const result = mergeEventCartTickets({ tickets: [{ id: 1, quantity: 5 }], items: [] }, [{ id: 1, quantity: 1, maxQuantity: 3 }]);
  ...
  test("prefers a newer local cart and repairs a stale tab session snapshot", () => {
    const slug = "evento-multitab"; const cartKey = `cutinapp_checkout_${slug}`; const now = Date.now();
    const staleCart = { tickets: [{ id: 1, quantity: 3 }], items: [], savedAt: now - 1000 };
    const freshCart = { tickets: [{ id: 1, quantity: 1 }], items: [], savedAt: now };
    window.sessionStorage.setItem(cartKey, JSON.stringify(staleCart)); window.localStorage.setItem(cartKey, JSON.stringify(freshCart));
    expect(readEventCart(slug)).toEqual(freshCart); expect(JSON.parse(window.sessionStorage.getItem(cartKey))).toEqual(freshCart);
  });

  test("prefers shared local cart when cross-tab writes have the same timestamp", () => {
    const slug = "evento-multitab-mesmo-ms"; const cartKey = `cutinapp_checkout_${slug}`; const now = Date.now();
    const sessionCart = { tickets: [{ id: 1, quantity: 3 }], items: [], savedAt: now };
    const sharedCart = { tickets: [{ id: 1, quantity: 1 }], items: [], savedAt: now };
    window.sessionStorage.setItem(cartKey, JSON.stringify(sessionCart)); window.localStorage.setItem(cartKey, JSON.stringify(sharedCart));
    expect(readEventCart(slug)).toEqual(sharedCart); expect(JSON.parse(window.sessionStorage.getItem(cartKey))).toEqual(sharedCart);
  });

  test("shared clear tombstone prevents another tab from resurrecting a stale cart", () => {
    const slug = "evento-limpo-em-outra-aba"; const cartKey = `cutinapp_checkout_${slug}`; const now = Date.now();
    window.sessionStorage.setItem(cartKey, JSON.stringify({ tickets: [{ id: 8, quantity: 2 }], items: [], savedAt: now - 1000 }));
    window.localStorage.setItem(cartKey, JSON.stringify({ cleared: true, savedAt: now }));
    expect(readEventCart(slug)).toBeNull(); expect(window.sessionStorage.getItem(cartKey)).toBeNull();
  ...
  test("expires a cart with an implausible future timestamp instead of keeping it indefinitely", () => {
    const slug = "evento-futuro-invalido"; const cartKey = `cutinapp_checkout_${slug}`;
    const futureCart = { tickets: [{ id: 3, quantity: 1 }], items: [], savedAt: Date.now() + (10 * 60 * 1000) };
    window.sessionStorage.setItem(cartKey, JSON.stringify(futureCart)); window.localStorage.setItem(cartKey, JSON.stringify(futureCart));
    expect(readEventCart(slug)).toBeNull(); expect(window.sessionStorage.getItem(cartKey)).toBeNull(); expect(window.localStorage.getItem(cartKey)).toBeNull();
  });

  test("expires persisted carts without a valid timestamp instead of reviving legacy state", () => {
    const slug = "evento-sem-timestamp"; const cartKey = `cutinapp_checkout_${slug}`;
    const legacyCart = { tickets: [{ id: 6, quantity: 2 }], items: [] };
    window.sessionStorage.setItem(cartKey, JSON.stringify(legacyCart)); window.localStorage.setItem(cartKey, JSON.stringify(legacyCart));
  ...
  test("keeps draft recovery synchronized with cart changes", () => {
    ...
    writeCheckoutRecovery(slug, { selection: { tickets: [{ id: 1, quantity: 3 }] }, couponCode: "VIP10", paymentMethod: "card" });
    writeEventCart(slug, { eventId: 77, tickets: [{ id: 1, quantity: 1 }], items: [{ id: 9, quantity: 2 }] });
    expect(readCheckoutRecovery(slug)?.selection).toEqual({ tickets: [{ id: 1, quantity: 1 }], items: [{ id: 9, quantity: 2 }] });
    expect(readCheckoutRecovery(slug)?.couponCode).toBe("VIP10"); expect(readCheckoutRecovery(slug)?.paymentMethod).toBe("card");
  });

  test("clears draft recovery when the participant empties the cart", () => {
    const slug = "evento-vazio"; writeEventCart(slug, { tickets: [{ id: 5, quantity: 1 }], items: [] }); expect(readCheckoutRecovery(slug)).not.toBeNull();
    clearEventCart(slug); expect(readCheckoutRecovery(slug)).toBeNull();
  });
