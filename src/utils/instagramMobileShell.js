...
const prepareBottomNav = () => {
  ...
  children.sort((a, b) => rank(a) - rank(b)).forEach((node) => nav.appendChild(node));
  ...
  Array.from(nav.children).forEach((node) => {
    ...
    if (icon) icon.className = iconFor(route, label);
    if (!node.getAttribute("aria-label") && label) node.setAttribute("aria-label", label);
    node.setAttribute("title", label || "Navegação");
  });
};

const ensureTopActions = () => {
  const navbar = document.querySelector(".cut-capability-nav .cut-navbar__inner");
  if (!navbar || navbar.querySelector(".cut-instagram-mobile-actions")) return;
  ...
  if (brand) {
    brand.setAttribute("href", "/event");
    brand.setAttribute("aria-label", "Cutinapp — Eventos");
  }
};
