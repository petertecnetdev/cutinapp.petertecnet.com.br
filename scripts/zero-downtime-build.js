...
function processIsAlive(pid) { if (!Number.isInteger(pid) || pid <= 0) return false; try { process.kill(pid, 0); return true; } catch (error) { return error?.code === 'EPERM'; } }
function readLockOwner() { try { return JSON.parse(fs.readFileSync(path.join(buildLockPath, 'owner.json'), 'utf8')); } catch (_) { return null; } }
function removeStaleBuildLock() { try { const stat = fs.statSync(buildLockPath); const owner = readLockOwner(); const ageMs = Date.now() - stat.mtimeMs; if ((owner && !processIsAlive(Number(owner.pid))) || ageMs > staleLockMs) { fs.rmSync(buildLockPath, { recursive: true, force: true }); return true; } } catch (error) { if (error?.code === 'ENOENT') return true; } return false; }
function acquireBuildLock() { const deadline = Date.now() + lockWaitMs; while (true) { try { fs.mkdirSync(buildLockPath); fs.writeFileSync(path.join(buildLockPath, 'owner.json'), JSON.stringify({ pid: process.pid, startedAt: new Date().toISOString(), appRoot })); return () => { const owner = readLockOwner(); if (!owner || Number(owner.pid) === process.pid) fs.rmSync(buildLockPath, { recursive: true, force: true }); }; } catch (error) { if (error?.code !== 'EEXIST') throw error; if (removeStaleBuildLock()) continue; if (Date.now() >= deadline) throw new Error('Timed out waiting for Cutinapp frontend build lock.'); sleep(1000); } } }
